...
export function buildPrintHtml(pages: string[], elements: DraggableElementData[], pageSettings: PageStyleSettings): string {
    ...
    const pageStyles = `
        body { 
            ...
            margin: 0;
            background-color: #eee; /* Preview background */
        }
    ...
    const pagesHtml = pages.map((pageHtml, index) => {
        ...
        return `
            <div class="print-page-container" style="width: ${pageSettings.orientation === 'portrait' ? (paperSizeForCss === 'A4' ? '210mm' : '182mm') : '297mm'}; height: ${pageSettings.orientation === 'portrait' ? '297mm' : '210mm'};">
...
export function buildContinuousPrintHtml(pages: string[], elements: DraggableElementData[], pageSettings: PageStyleSettings): string {
    const combinedHtml = pages.join('');
    // Note: Absolute positioned elements are hard to place in continuous mode effectively without page contexts.
    // For "Continuous View", we typically assume the user wants the flow of text.
    // We will omit draggables for continuous print view to avoid overlapping issues, or user should use Paged view.
    ...
    const styles = `
        ...
        .continuous-content {
         ...
         p, table, .question-item {
            margin-top: 0;
            margin-bottom: ${pageSettings.questionSpacing}pt;
        }
        table { width: 100%; border-collapse: collapse; }
        td { width: 25%; vertical-align: top; padding: 0 1em 0 0; }
        
        @media print {
            .continuous-content { width: 100%; max-width: none; }
        }
    `;
